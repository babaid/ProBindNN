...
def comparator2(model,mlp,  dataset,n, filename:str,device="cuda"):
    xs = []
    ys = []
    model.eval()
    ...
    for i in tqdm(range(n)):
        x, y, = dataset[i]["mutated"].to(device), dataset[i]["non_mutated"].to(device)
        ddg = dataset[i]["mutated"].ddg
        dg1 = model(x, x.batch).squeeze()
        dg2 = model(y, y.batch).squeeze()
        out = mlp(dg1-dg2)
          
        xs.append(out.squeeze().item())
        ys.append(ddg)
    xs = np.array(xs)
    ys = np.array(ys)
    ...
    ax = fig.add_subplot(111)
    ax.scatter(xs, ys, s=1, marker=".", label = "r = {}".format(corr))
    #ax.set_ylim(-2, 4)
    #ax.set_xlim(-2, 4)
    ax.set_xlabel("Predicted $\Delta \Delta G$")
    ax.set_ylabel("Experimental $\Delta \Delta G$")
